-- Migration 20260718000000_p0_server_authoritative_transaksi_mitra.sql
-- Resolves TASK-SEC-006 & TASK-SEC-001 by moving financial calculation
-- from the browser to a SECURITY DEFINER RPC.

BEGIN;

CREATE OR REPLACE FUNCTION public.save_transaksi_mitra_v2(payload jsonb)
...
-- Revoke execute from public/anon just in case
REVOKE EXECUTE ON FUNCTION public.save_transaksi_mitra_v2(jsonb) FROM PUBLIC, anon, authenticated;
GRANT EXECUTE ON FUNCTION public.save_transaksi_mitra_v2(jsonb) TO authenticated;
