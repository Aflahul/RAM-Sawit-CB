-- Sawit CB - P0: Update RPC create_pembayaran_pabrik_batch agar memakai berat_netto_pabrik_kg
-- dan menyimpan snapshot berat baru di pembayaran_pabrik_item.
--
-- Perubahan aman:
--   - COALESCE(tm.berat_netto_pabrik_kg, tm.tonase) → berat netto dari pabrik jika ada,
--     fallback ke tonase lama agar transaksi sebelum migration tetap berfungsi.
--   - Tambah kolom berat_netto_snapshot dan berat_dibayar_snapshot di pembayaran_pabrik_item.

BEGIN;

-- P0: Tambah kolom snapshot berat baru di pembayaran_pabrik_item (aman jika sudah ada)
ALTER TABLE public.pembayaran_pabrik_item
  ADD COLUMN IF NOT EXISTS berat_netto_snapshot  numeric(15,2),
  ADD COLUMN IF NOT EXISTS berat_dibayar_snapshot numeric(15,2);

-- P0: Tambah kolom snapshot berat di pembayaran_mitra_kwitansi_item
ALTER TABLE public.pembayaran_mitra_kwitansi_item
  ADD COLUMN IF NOT EXISTS berat_netto_snapshot       numeric(15,2),
  ADD COLUMN IF NOT EXISTS potongan_snapshot          numeric(15,2),
  ADD COLUMN IF NOT EXISTS berat_dibayar_snapshot     numeric(15,2),
  ADD COLUMN IF NOT EXISTS pakai_sewa_armada_snapshot boolean,
  ADD COLUMN IF NOT EXISTS biaya_sewa_armada_snapshot numeric(15,2);

-- P0: Tambah kolom total sewa armada di header kwitansi
