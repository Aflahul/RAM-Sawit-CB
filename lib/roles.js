...
export function canManageFinance(role) {
  return [ROLES.OWNER, ROLES.SUPER_ADMIN, ROLES.ADMIN_KEUANGAN, ROLES.ADMIN_OPERASIONAL].includes(normalizeRole(role));
}

export function canRecordMitraPayment(role) {
...
export function isOperationalAdmin(role) {
  return normalizeRole(role) === ROLES.ADMIN_OPERASIONAL;
}
