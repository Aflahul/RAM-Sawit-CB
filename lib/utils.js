...
/**
 * Format rentang tanggal tampilan.
 * @param {string|Date} dateFrom
 * @param {string|Date} dateTo
 * @returns {string} contoh: "07 s/d 10-07-2025"
 */
export function formatDateRangeDisplay(dateFrom, dateTo) {
  const from = getDateParts(dateFrom);
  const to = getDateParts(dateTo);
  if (!from || !to) return '-';

  const fromFull = `${from.day}-${from.month}-${from.year}`;
  const toFull = `${to.day}-${to.month}-${to.year}`;

  if (fromFull === toFull) return toFull;
  if (from.year === to.year && from.month === to.month) return `${from.day} s/d ${toFull}`;
  if (from.year === to.year) return `${from.day}-${from.month} s/d ${toFull}`;
  return `${fromFull} s/d ${toFull}`;
}

export function formatDateTimeDisplay(date, { seconds = true } = {}) {
  const d = new Date(date);
  if (!date || Number.isNaN(d.getTime())) return '-';

  return `${formatDateDisplay(d)}, ${d.toLocaleTimeString('id-ID', {
    hour: '2-digit',
    minute: '2-digit',
...
/**
 * Format tanggal ke format tampilan standar.
 * @param {string|Date} date
 * @param {object} options
 * @returns {string} contoh: "07-07-2025"
 */
export function formatTanggal(date, options = null) {
...
export function getTimestampMs(date) {
  const d = new Date(date);
  return !date || Number.isNaN(d.getTime()) ? 0 : d.getTime();
}
