...
export function resolveBiayaSewaArmada(row) {
  ...
  if (row?.pakai_sewa_armada_bl) {
    const kotor = row?.biaya_sewa_armada_kotor != null ? toNumber(row.biaya_sewa_armada_kotor) : Math.round(resolveBeratNettoPabrik(row) * (toNumber(row.tarif_sewa_angkut_per_kg_snapshot) || 0));
    return Math.max(0, kotor - (toNumber(row.nominal_perongkosan_snapshot) || 0));
  }
  return 0;
}
...
export function kalkulasiTransaksiMitra(row, hargaPabrikOverride, feeOwnerOverride) {
  ...
  const perongkosan = toNumber(row?.nominal_perongkosan_snapshot);

  const biayaSewaArmadaKotor = pakaiSewaArmada ? Math.round(beratNetto * sewaAngkutPerKg) : 0;
  const sewaArmadaTotal  = pakaiSewaArmada ? Math.max(0, biayaSewaArmadaKotor - perongkosan) : 0;
 ...
 */
export function hitungSewaArmadaCB({ isArmadaCB, beratNettoPabrikKg, tarifSewaAngkut = 0, nominalPerongkosan = 0 }) {
  ...
  const biayaSewaArmadaTotal = Math.max(0, biayaSewaArmadaKotor - nominalPerongkosan);

  return {
    pakaiSewaArmada: true,
    biayaSewaArmadaKotor,
    biayaSewaArmadaTotal,
  };
}
...
export function resolveEffectiveMitraFeeSnapshot({ mitraId, tanggal, mitras = [], feeHistories = [] }) {
  ...
  if (shouldPreferMaster) {
    return {
      ...
      nominalPerongkosan: toNumber(fallbackMitra?.nominal_perongkosan),
  ...
  return {
    fee: historyFee,
    tarifSewaAngkut: toNumber(history?.tarif_sewa_angkut_per_kg),
    nominalPerongkosan: toNumber(history?.nominal_perongkosan),
