...
//   biaya_sewa_armada  = berat_netto_pabrik_kg x TARIF_SEWA_ARMADA_BL_PER_KG
//
...
/** Tarif sewa armada BL/SL per kg berat NETTO (bukan berat dibayar). */
export const TARIF_SEWA_ARMADA_BL_PER_KG = 150;
// TODO: pindah ke tabel pengaturan/tarif history saat P3.
...
// ---------------------------------------------------------------------------
// Resolver sewa armada BL/SL
// ---------------------------------------------------------------------------

/**
 * Apakah transaksi ini kena biaya sewa armada BL/SL?
 */
...
/**
 * Total biaya sewa armada BL/SL.
 * Dihitung dari BERAT NETTO (bukan berat dibayar).
 */
export function resolveBiayaSewaArmada(row) {
  if (row?.biaya_sewa_armada_total != null) return toNumber(row.biaya_sewa_armada_total);
  if (row?.pakai_sewa_armada_bl) {
    return Math.round(resolveBeratNettoPabrik(row) * TARIF_SEWA_ARMADA_BL_PER_KG);
  }
  return 0;
}
...
export function kalkulasiTransaksiMitra(row, hargaPabrikOverride, feeOwnerOverride) {
  ...
  const sewaArmadaTotal  = pakaiSewaArmada
    ? Math.round(beratNetto * TARIF_SEWA_ARMADA_BL_PER_KG)
    : 0;
...
// ---------------------------------------------------------------------------
// Deteksi sewa armada BL/SL
// ---------------------------------------------------------------------------

/**
 * Apakah kode mitra termasuk armada BL/SL?
 * MVP: deteksi dari prefix kode mitra.
 * TODO (P3): ganti dengan flag is_armada_internal di master_mitra.
 */
export function isMitraBLSL(mitra) {
  const kode = String(mitra?.kode || '').toUpperCase().trim();
  return kode.startsWith('BL') || kode.startsWith('SL');
}
 ...
 *
 * @param {object} params
 * @param {object} params.mitraTransaksi   - objek master_mitra untuk mitra transaksi
 * @param {object} params.mitraAfiliasiSopir - objek master_mitra afiliasi sopir/armada
 ...
 */
export function hitungSewaArmadaBL({ mitraTransaksi, mitraAfiliasiSopir, beratNettoPabrikKg }) {
  const berat = toNumber(beratNettoPabrikKg);

  // Mitra transaksi sudah BL/SL — tidak kena biaya sewa ke diri sendiri
  if (isMitraBLSL(mitraTransaksi)) {
    return { pakaiSewaArmada: false, biayaSewaArmadaPerKg: 0, biayaSewaArmadaTotal: 0 };
  }

  // Sopir/armada tidak berafiliasi ke BL/SL
  if (!isMitraBLSL(mitraAfiliasiSopir)) {
    return { pakaiSewaArmada: false, biayaSewaArmadaPerKg: 0, biayaSewaArmadaTotal: 0 };
  }

  // Mitra luar memakai armada BL/SL → kena sewa
  return {
    pakaiSewaArmada: true,
    biayaSewaArmadaPerKg: TARIF_SEWA_ARMADA_BL_PER_KG,
    biayaSewaArmadaTotal: Math.round(berat * TARIF_SEWA_ARMADA_BL_PER_KG),
  };
}
