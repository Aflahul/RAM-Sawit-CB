...
//   sewa_armada_kotor  = berat_netto_pabrik_kg x tarif_sewa_mitra_per_kg
//   potongan_sewa_akhir = max(sewa_armada_kotor - dana_operasional_dibayar_mitra, 0)
//
...
/**
 * Sewa kotor Armada CB sebelum dikurangi Dana Operasional yang telah dibayar
 * langsung oleh Mitra kepada sopir.
 */
export function resolveBiayaSewaArmadaKotor(row) {
  if (row?.pakai_sewa_armada_bl) {
    if (row?.biaya_sewa_armada_standar_snapshot != null) {
      return toNumber(row.biaya_sewa_armada_standar_snapshot);
    }
    ...
    const finalSewa = toNumber(row?.biaya_sewa_armada_total ?? row?.biaya_sewa_armada_snapshot);
    const danaOperasional = resolveDanaOperasionalTrip(row);
    return isDanaOperasionalDibayarMitra(row) ? finalSewa + danaOperasional : finalSewa;
  }
  return 0;
}

/**
 * Apakah Dana Operasional diserahkan langsung oleh Mitra kepada sopir.
 * Snapshot item kwitansi selalu menjadi sumber utama agar dokumen yang sudah
 * terbit tidak berubah. Transaksi aktif lama yang belum dibayar mengikuti
 * keputusan Owner terbaru sampai tersimpan ulang oleh trigger database.
 */
export function isDanaOperasionalDibayarMitra(row) {
  if (row?.is_kwitansi_snapshot && row?.dana_operasional_dibayar_mitra_snapshot != null) {
    return Boolean(row.dana_operasional_dibayar_mitra_snapshot);
  }

  const isActiveDirectFund = Boolean(
    (row?.menggunakan_armada_cb_snapshot ?? row?.pakai_sewa_armada_bl)
    && row?.catat_dana_operasional_trip !== false
    && row?.status !== 'dibatalkan'
  );

  // Keputusan Owner: pada transaksi aktif, Dana Operasional Armada CB selalu
  // sudah dibayar langsung oleh Mitra sebelum berangkat. Timestamp pembayaran
  // sopir lama tidak boleh mengubahnya menjadi biaya/kas keluar CB.
  if (isActiveDirectFund) return true;

  if (row?.dana_operasional_dibayar_mitra_snapshot != null) {
    return Boolean(row.dana_operasional_dibayar_mitra_snapshot);
  }
  if (row?.dana_operasional_dibayar_mitra != null) {
    return Boolean(row.dana_operasional_dibayar_mitra);
  }
...
/**
 * Potongan akhir sewa pada hak Mitra.
 * Rumus Owner: (berat netto x tarif sewa Mitra/kg) - Dana Operasional yang
 * sudah dibayar langsung oleh Mitra kepada sopir.
 */
export function resolveBiayaSewaArmada(row) {
  const sewaKotor = resolveBiayaSewaArmadaKotor(row);
  if (!isDanaOperasionalDibayarMitra(row)) {
    return toNumber(row?.biaya_sewa_armada_total ?? row?.biaya_sewa_armada_snapshot ?? sewaKotor);
  }

  return Math.max(0, sewaKotor - resolveDanaOperasionalTrip(row));
}

/**
 * Dana Operasional Trip Armada CB yang dicatat untuk perjalanan ini.
 * Dana ini dibayar langsung oleh Mitra kepada sopir sebelum berangkat dan
 * menjadi pengurang sewa kotor Armada CB pada kwitansi.
 */
export function resolveDanaOperasionalTrip(row) {
  const menggunakanArmadaCb = Boolean(
    row?.menggunakan_armada_cb_snapshot ?? row?.pakai_sewa_armada_bl
  );
  const catatDana = Boolean(
    row?.catat_dana_operasional_trip
    ?? (toNumber(row?.dana_operasional_trip_snapshot ?? row?.total_biaya_sopir_cb_snapshot) > 0)
  );

  if (!menggunakanArmadaCb || !catatDana) return 0;

  const snapshot = toNumber(row?.dana_operasional_trip_snapshot);
  if (snapshot > 0) return snapshot;

  const legacySnapshot = toNumber(row?.total_biaya_sopir_cb_snapshot);
  if (legacySnapshot > 0) return legacySnapshot;

  return toNumber(row?.dana_operasional_trip);
}
...
export function kalkulasiTransaksiMitra(row, hargaPabrikOverride, feeOwnerOverride) {
  ...
  const danaDibayarMitra = isDanaOperasionalDibayarMitra(row);
  const sewaArmadaTotal = pakaiSewaArmada
    ? Math.max(0, biayaSewaArmadaKotor - (danaDibayarMitra ? danaOperasionalTrip : 0))
    : 0;
  ...
  const marginArmadaSetelahDanaTrip = sewaArmadaTotal;
...
export function resolveEffectiveMitraFeeSnapshot({ mitraId, tanggal, mitras = [], feeHistories = [] }) {
  ...
  if (shouldPreferMaster) {
    return {
      ...
      danaOperasionalTrip: toNumber(fallbackMitra?.dana_operasional_trip),
  ...
  return {
    ...
    danaOperasionalTrip: toNumber(history?.dana_operasional_trip ?? fallbackMitra?.dana_operasional_trip),
