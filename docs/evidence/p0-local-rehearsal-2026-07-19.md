...
Smoke UI dijalankan tanpa melewati batas Production: proses Next.js lokal menerima environment staging hanya melalui process override dan memiliki safety guard terhadap project ref staging. Test memakai akun Super Admin sintetis ber-password acak, memeriksa negative/positive login dan lima route utama, lalu menutup browser/server dan menghapus profile serta Auth user dalam cleanup. Sesudah run, port test tidak lagi listen, script sementara dihapus, dan Auth staging kembali kosong.
